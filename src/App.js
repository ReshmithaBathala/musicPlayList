...
class App extends Component {
  ...
  deleteTrack = id => {
    ...
    const filteredDeleted = musicList.filter(eachL => eachL.id !== id)
    this.setState({
      musicList: filteredDeleted,
    })
  }

  renderFilteredList = () => {
    const {musicInput, musicList} = this.state
    const filteredList = musicList.filter(eachVal =>
  ...
  render() {
    ...
    return (
      <div className="music-main-container">
        <div className="music-playlist-img-container">
          <h1 className="singer-heading">Ed Sheeran</h1>
          <p className="singer-paragraph">Singer</p>
        </div>
        <div className="input-song-heading-container">
        ...
        {filteredListLength ? (
          <ul className="song-track-ul-list">
            {filteredMusicList.map(eachTrack => (
              <SongTrack
                key={eachTrack.id}
